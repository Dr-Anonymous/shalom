...
function deleteDB(){ set(ref(db), null); }

function gotStream(stream) {
	localVideo.srcObject = stream;
	localStream = stream;
	localVideo.play().then(_ => {})
		.catch(error => {});
...
function onIceCandidate(pc, event) {
	if (event.candidate == null) {
		if (one) set(ref(db, '0/'), JSON.stringify(pc.localDescription));
		else if (three) set(ref(db, '2/'), JSON.stringify(pc.localDescription));
		else set(ref(db, '1/'), JSON.stringify(pc.localDescription));
	}
...
audioInputSelect.onchange = deleteDB;
audioOutputSelect.onchange = changeAudioDestination;
videoSelect.onchange = deleteDB;
